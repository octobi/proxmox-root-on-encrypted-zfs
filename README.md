...
```bash
curl -fsSL https://raw.githubusercontent.com/octobi/proxmox-root-on-encrypted-zfs/main/src/cli.ts | sudo sh -s --
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/octobi/proxmox-root-on-encrypted-zfs/main/src/cli.ts | sh -s --
...
sudo  NON_INTERACTIVE=true \
      ...
      ROOT_AUTHORIZED_KEYS_URL=https://github.com/octobi.keys \
      /tmp/deno-range*/bin/deno run --reload=https://raw.githubusercontent.com --unstable --allow-all \
        https://raw.githubusercontent.com/octobi/proxmox-root-on-encrypted-zfs/main/src/cli.ts \
        debian
```
